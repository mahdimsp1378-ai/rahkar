...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html,body,input,select,textarea,button,a,label{font-family:var(--font)!important}body{margin:0;background:radial-gradient(circle at top right,#dff8ff,transparent 34%),linear-gradient(135deg,#eff6ff,#f8fafc);color:var(--ink);min-height:100vh}button,input,select,textarea{font:inherit!important;letter-spacing:0}::placeholder{font-family:var(--font);color:#94a3b8}.wrap{max-width:1120px;margin:0 auto;padding:28px 18px 56px}.top{display:flex;justify-content:space-between;align-items:center;gap:16px;margin-bottom:26px}.brand{font-weight:900;font-size:24px;color:var(--blue)}.back{border:1px solid var(--line);background:#fff;border-radius:999px;padding:10px 16px;color:var(--blue);text-decoration:none;font-weight:800}.hero{display:grid;grid-template-columns:1fr 1fr;gap:22px;align-items:stretch}.panel{background:rgba(255,255,255,.92);border:1px solid var(--line);box-shadow:0 24px 70px rgba(37,99,235,.13);border-radius:28px;padding:28px}.dark{background:linear-gradient(145deg,#061d37,#0d355d);color:#fff}.dark p{color:#dbeafe}.hero h1{font-size:clamp(30px,5vw,56px);line-height:1.35;margin:0 0 14px;font-weight:900}.hero p{color:var(--muted);line-height:2;font-size:16px}.badge{display:inline-flex;gap:8px;align-items:center;background:#e0f2fe;color:#075985;border-radius:999px;padding:8px 13px;font-weight:900;margin-bottom:14px}.form{display:grid;gap:14px}.field{display:grid;gap:7px}.field label{font-weight:900;color:#0f172a}.field input,.field select,.field textarea{width:100%;border:1px solid #cbd5e1;border-radius:16px;padding:13px 14px;background:#fff;color:var(--ink);outline:none}.field input:focus,.field select:focus,.field textarea:focus{border-color:#0f4fcb;box-shadow:0 0 0 4px rgba(15,79,203,.11)}.field textarea{min-height:130px;resize:vertical}.hint{color:var(--muted);font-size:13px}.row{display:grid;grid-template-columns:1fr 1fr;gap:12px}.send{border:0;border-radius:18px;padding:15px 18px;background:linear-gradient(135deg,var(--blue),var(--cyan));color:#fff;font-weight:900;cursor:pointer;box-shadow:0 16px 35px rgba(37,99,235,.25)}.result{display:none;margin-top:16px;border:1px solid #bae6fd;background:#f0f9ff;border-radius:20px;padding:18px;line-height:2}.result.show{display:block}.result.error{border-color:#fecaca;background:#fff1f2;color:#9f1239}.cta{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}.cta a{border-radius:999px;padding:10px 15px;text-decoration:none;font-weight:900}.primary{background:var(--blue);color:#fff}.secondary{background:#fff;color:var(--blue);border:1px solid var(--line)}.native-file{position:absolute;inline-size:1px;block-size:1px;opacity:0;pointer-events:none}.file-picker{display:flex;justify-content:space-between;align-items:center;gap:12px;border:1px dashed #93c5fd;border-radius:16px;background:#f8fbff;padding:13px 14px;cursor:pointer;color:#0f4fcb;font-weight:900}.file-picker small{font-weight:700;color:#64748b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.file-picker span{background:#dbeafe;border-radius:999px;padding:7px 12px;white-space:nowrap}.error-text{color:#dc2626;font-size:13px;font-weight:800;display:none}.field.invalid .error-text{display:block}.field.invalid input,.field.invalid select,.field.invalid textarea{border-color:#ef4444;box-shadow:0 0 0 4px rgba(239,68,68,.08)}
    ...
    html{min-width:320px;overflow-x:hidden;-webkit-text-size-adjust:100%;text-size-adjust:100%}body{min-width:320px;min-height:100dvh;overflow-x:hidden}button,a,input,select,textarea,label{touch-action:manipulation}input,select,textarea{font-size:16px!important}.back,.send,.chat-form button,.cta a{min-height:48px;display:inline-flex;align-items:center;justify-content:center}
    @media(max-width:600px){.wrap{padding:calc(14px + env(safe-area-inset-top,0px)) 10px calc(24px + env(safe-area-inset-bottom,0px))}.top{margin-bottom:14px}.brand{font-size:20px}.back{padding:8px 14px}.panel{padding:18px 14px;border-radius:20px}.dark{display:none}.hero h1{font-size:30px}.field input,.field select,.field textarea{min-height:48px;border-radius:13px}.field textarea{min-height:120px}.topic-help,.hint{font-size:13px}.chat-card{border-radius:18px}.chat-head{padding:16px 14px;flex-direction:column}.quota{align-self:stretch;text-align:center}.messages{height:auto;min-height:calc(100dvh - 315px);max-height:calc(100dvh - 250px);padding:12px;overscroll-behavior:contain}.message{max-width:94%;padding:12px 13px}.chat-form{position:sticky;bottom:0;padding:10px 12px calc(10px + env(safe-area-inset-bottom,0px));gap:8px}.chat-form textarea{min-height:48px;max-height:96px;resize:none}.limit-box{padding:16px 14px}.cta{flex-direction:column}.cta a{width:100%}}
    @media(max-width:360px){.wrap{padding-inline:7px}.panel{padding-inline:11px}.brand{font-size:18px}.message{max-width:98%}}
    @media(orientation:landscape) and (max-height:520px){.messages{min-height:210px;max-height:58dvh}.chat-head{flex-direction:row}.panel{padding:14px}}
...
<body>
  <main class="wrap">
    ...
    <section class="hero">
      ...
      <form class="panel form" id="ai-form" novalidate>
        ...
        <div class="row"><div class="field"><label for="size">اندازه سازمان</label><select id="size" name="size"><option>کمتر از ۵۰ نفر</option><option>۵۰ تا ۲۰۰ نفر</option><option>بیش از ۲۰۰ نفر</option></select></div><div class="field"><label for="topic">حوزه اصلی مشاوره</label><select id="topic" name="topic" required><option value="">انتخاب حوزه اصلی</option><option>حقوقی</option><option>مالی</option><option>منابع انسانی</option><option>تحول دیجیتال و سامانه‌های هوشمند</option></select><span class="error-text">لطفاً حوزه اصلی را انتخاب کنید.</span></div></div>
        <div class="field"><label for="subtopic">زیرموضوع</label><select id="subtopic" name="subtopic" required disabled><option value="">ابتدا حوزه اصلی را انتخاب کنید</option></select><span class="topic-help">با انتخاب حوزه، موضوعات تخصصی مرتبط نمایش داده می‌شوند.</span><span class="error-text">لطفاً زیرموضوع را انتخاب کنید.</span></div>
        <div class="field conditional-field" id="other-topic-field" hidden><label for="subtopicOther">موضوع موردنظر شما</label><input id="subtopicOther" name="subtopicOther" maxlength="160" placeholder="موضوع را کوتاه و دقیق بنویسید..." /><span class="error-text">لطفاً موضوع موردنظر را بنویسید.</span></div>
  ...
  <script>
    ...
    var subtopicOther = document.getElementById('subtopicOther');
    ...
    var topicOptions = {
      'حقوقی': ['قراردادها و تعهدات', 'قوانین کار و روابط کار', 'دعاوی و حل اختلاف', 'ثبت شرکت، مجوزها و حاکمیت شرکتی', 'مالکیت فکری و محرمانگی', 'انطباق قانونی و مدیریت ریسک حقوقی', 'سایر'],
      'مالی': ['بودجه‌ریزی و کنترل هزینه', 'حسابداری و گزارش‌های مالی', 'مالیات و بیمه', 'جریان نقد و تأمین مالی', 'تحلیل سرمایه‌گذاری و امکان‌سنجی', 'کنترل داخلی و ریسک مالی', 'حقوق و دستمزد از منظر مالی', 'سایر'],
      'منابع انسانی': ['جذب و استخدام', 'ارزیابی عملکرد و بهره‌وری', 'حقوق، مزایا و جبران خدمات', 'آموزش و توسعه کارکنان', 'ساختار سازمانی و شرح شغل', 'نگرش‌سنجی و تجربه کارکنان', 'جانشین‌پروری و مسیر شغلی', 'روابط کار و نگهداشت کارکنان', 'سایر'],
      'تحول دیجیتال و سامانه‌های هوشمند': ['خودکارسازی فرایندهای سازمانی', 'داشبورد مدیریتی و هوش تجاری', 'طراحی سامانه اختصاصی', 'ایجنت‌ها و راهکارهای هوش مصنوعی', 'یکپارچه‌سازی داده و گزارش‌سازی', 'سامانه هوشمند منابع انسانی', 'بازطراحی و دیجیتالی‌سازی فرایندها', 'سایر']
    };
    function updateSubtopics() {
      var options = topicOptions[topic.value] || [];
      subtopic.innerHTML = '<option value="">' + (options.length ? 'انتخاب زیرموضوع' : 'ابتدا حوزه اصلی را انتخاب کنید') + '</option>' + options.map(function (item) { return '<option>' + item + '</option>'; }).join('');
      subtopic.disabled = !options.length;
      otherTopicField.hidden = true;
      subtopicOther.required = false;
      subtopicOther.value = '';
    }
    topic.addEventListener('change', updateSubtopics);
    subtopic.addEventListener('change', function () {
      var isOther = subtopic.value === 'سایر';
      otherTopicField.hidden = !isOther;
      subtopicOther.required = isOther;
      if (!isOther) subtopicOther.value = '';
      if (isOther) subtopicOther.focus();
    });
    ...
    form.addEventListener('submit', async function (event) {
      ...
      try {
        ...
        var selectedSubtopic = form.subtopic.value === 'سایر' ? form.subtopicOther.value : form.subtopic.value;
        document.getElementById('chat-context').textContent = form.org.value + ' · ' + form.topic.value + ' · ' + selectedSubtopic;
