import './marketing-enhancements.js';
...
function dashboardCard(type, title, subtitle, body, insight) {
  return `<article class="rk-ai-dashboard-card rk-ai-${type}">
    <header><span>${icon(type === 'people' ? 'brain' : type === 'process' ? 'bot' : 'spark')}</span><div><b>${title}</b><small>${subtitle}</small></div></header>
    ${body}
    <footer><span>${icon('spark')}</span><div><b>تحلیل هوشمند</b><p>${insight}</p></div></footer>
  </article>`;
}

function tripleDashboardMarkup() {
  const people = dashboardCard('people','سرمایه انسانی','مشارکت، ریسک خروج و ترکیب کارکنان',`
    ...
    <div class="rk-mini-bars">${[['عملیات',88],['فروش',72],['پشتیبانی',61]].map(([l,v])=>`<div><small>${l}</small><span><i style="--v:${v}%"></i></span><b>${v}</b></div>`).join('')}</div>
  `,'افت مشارکت در واحد عملیات با افزایش اضافه‌کار هم‌زمان شده است.');

  const process = dashboardCard('process','فرایندهای هوشمند','وضعیت گردش‌کار و زمان پاسخ',`
  ...
  `,'گلوگاه اصلی در مرحله بررسی مدیر میانی است؛ خودکارسازی تأیید اولیه پیشنهاد می‌شود.');

  const payroll = dashboardCard('payroll','حقوق و بهره‌وری','پرداخت، اضافه‌کار و روند هزینه',`
  ...
  return `<section class="rk-enhance-section rk-triple-dashboard" id="dashboards" data-rk-final-dashboard>
    <div class="rk-section-heading rk-split-heading"><div><span>سه نمای تحلیلی</span><h2>داشبوردهای هوشمند سازمان</h2></div><p>سه داشبورد مکمل برای سرمایه انسانی، فرایندها و حقوق و بهره‌وری؛ هر کدام همراه با تحلیل و پیشنهاد هوش مصنوعی.</p></div>
    <div class="rk-ai-dashboard-grid">${people}${process}${payroll}</div>
  </section>`;
}

function replaceDashboard() {
  const current = document.querySelector('#dashboards');
  if (!current || current.hasAttribute('data-rk-final-dashboard')) return;
  current.outerHTML = tripleDashboardMarkup();
}
